...
\* This statistics may not be up to date (specially the GCC one). I wanted
to count them, but the GCC project has 74853 files (v7.3.0), so after an hour of
counting with `cloc`, I decided to give up and google it. I didn't even attempt
to count the Linux kernel.
